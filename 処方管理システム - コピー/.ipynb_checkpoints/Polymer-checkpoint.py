...
layout = [[sg.Text("処方管理システム / Polymer", font=("Arial",10,"bold"))],
          ...
          [
              ...
              for kosu_name,kosu_key in kosu_dict.items()
          ],
...
while True:
    ...
    if event == "実行":
        ...
        composition["Conc"]= float(values["conc"])
        ...
        #compositionからfunctionを計算
        functional_df = composition_to_function(monomer_df,composition_df)

        #GMA変性アルゴリズムの実行----------------------------------------------

        functional_df = functional_df.reset_index(drop=True)
        carboxy = functional_df["carboxy"]
        epoxy = functional_df["epoxy"]
        hydroxy = functional_df["hydroxy"]

        for i in range(len(epoxy)):
            epo_car_dist = epoxy[i] - carboxy[i]

            if epo_car_dist >0:
                hydroxy[i] = hydroxy[i]+carboxy[i]
                epoxy[i] = epo_car_dist
                carboxy[i] = 0

            elif epo_car_dist <0:
                hydroxy[i] = hydroxy[i]+epoxy[i]
                epoxy[i] = 0
                carboxy[i] = -epo_car_dist
        ...
        functional_df["carboxy"] = carboxy
        functional_df["epoxy"] = epoxy
        functional_df["hydroxy"] = hydroxy
    ...
    if event == "データ読込":
        ...
        window["conc"].update(target_row_dict["Conc"])
